...
from dada_mall.utils.models import BaseModel
...
class SPU(BaseModel):
    ...
    catalog_id = models.IntegerField(verbose_name='商品类别')
...
class SPUSaleAttr(BaseModel):
    ...
    SPU销售属性表
    """
    spu_id = models.IntegerField(verbose_name='spu_id')
...
class SaleAttrValue(BaseModel):
    ...
    销售属性值表
    """
    spu_sale_attr_id = models.IntegerField(verbose_name='销售属性id')
    ...
    def __str__(self):
        ...
        return '%s - %s' % (self.spu_sale_attr_id, self.name)
...
class SKUImage(BaseModel):
    ...
    SKU图片
    """
    sku_id = models.IntegerField(verbose_name='SKU id')
    ...
    def __str__(self):
        return '%s %s' % (self.sku_id, self.id)
...
class SPUSpec(BaseModel):
    ...
    def __str__(self):
        return '%s: %s' % (self.spu_id, self.name)
...
class SKUSpecValue(BaseModel):
    ...
    """
    sku_id = models.IntegerField(verbose_name='SKU id')
    spu_spec_id = models.IntegerField(verbose_name='spu_space_id')
    ...
    def __str__(self):
        return '%s: %s: %s' % (self.sku_id, self.spu_spec_id, self.name)
